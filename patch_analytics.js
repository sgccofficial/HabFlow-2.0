...
code = code.replace(/const ratio = completedCount \/ activeCount;\n\s*if \(completedCount === 0\) \{\n\s*colorClass = 'bg-red-400 dark:bg-red-500 shadow-sm';/g, `const ratio = completedCount / activeCount;
...
code = code.replace(/const tDays = h.targetDays \|\| \[0, 1, 2, 3, 4, 5, 6\];/, `const tDays = h.targetDays || [0, 1, 2, 3, 4, 5, 6];`); // just in case
